...
# Add RandOpt directory to sys.path
sys.path.append(os.path.join(os.getcwd(), "code/RandOpt"))
...
def parse_args():
    ...
    parser.add_argument("--robust_data", type=str, default="datasets/gsm8k_robust/test_robust.json",
                        help="Path to the robust test set")
    parser.add_argument("--method", type=str, choices=["standard", "antithetic", "orthogonal"], default="standard",
                        help="Sampling method for perturbations")
...
def run_sampling(args, engines, handler, train_prompts, train_datas, sampling_params):
    """Run perturbation sampling with various methods."""
    ...
    perf: Dict[Tuple[int, float, bool], float] = {}
    
    # Pre-generate unique seeds and sigmas
    if args.method == "antithetic":
        # Divide population size by 2 for pairs
        effective_pop = args.population_size // 2
        all_seeds = rng.choice(2**31, size=effective_pop, replace=False).tolist()
        all_sigmas = rng.choice(args.sigma_list, size=effective_pop).tolist()
        # Each seed gets both + and - versions
        all_runs = []
        for s, sig in zip(all_seeds, all_sigmas):
            all_runs.append((s, sig, False))
            all_runs.append((s, sig, True))
    else:
        ...
        all_runs = [(s, sig, False) for s, sig in zip(all_seeds, all_sigmas)]
    
    samples_evaluated, batch_idx = 0, 0
    
    while samples_evaluated < len(all_runs):
        batch_size = min(args.num_engines, len(all_runs) - samples_evaluated)
        batch = all_runs[samples_evaluated:samples_evaluated + batch_size]
        
        # Evaluate batch
        ray.get([engines[i].collective_rpc.remote("perturb_self_weights", args=(int(s), sig, neg)) 
                 for i, (s, sig, neg) in enumerate(batch)])
        
        outputs = ray.get([engines[i].generate.remote(train_prompts, sampling_params, use_tqdm=False) 
                          for i in range(len(batch))])
        
        ray.get([engines[i].collective_rpc.remote("restore_self_weights", args=(int(s), sig, neg)) 
                 for i, (s, sig, neg) in enumerate(batch)])
        
        # Process results
        rewards = []
        for i, (seed, sigma, neg) in enumerate(batch):
            r = handler.postprocess_outputs(outputs[i], train_datas)
            perf[(seed, sigma, neg)] = r
            rewards.append(r)
        
        samples_evaluated += len(batch)
        batch_idx += 1
        print(f"  Batch {batch_idx} | {samples_evaluated}/{len(all_runs)} | {['%.3f' % r for r in rewards]}")
    ...
    for (seed, sigma, neg), reward in perf.items():
...
def run_ensemble_evaluation(args, engines, handler, test_prompts, test_datas, top_k_perturbs, sampling_params, base_test):
    ...
    for batch_idx in range(total_batches):
        ...
        ray.get([engines[i].collective_rpc.remote("perturb_self_weights", args=(int(s), sig, neg)) 
                 for i, (s, sig, neg) in enumerate(batch_perturbs)])
...
def save_results(args, logging_dir, model_saves_dir, base_model_path, handler, 
    ...
    seeds_info = {
        ...
        "top_k_models": [
            {"rank": i+1, "seed": int(seed), "sigma": float(sigma), "negate": bool(neg), "train_reward": float(reward)}
            for i, ((seed, sigma, neg), reward) in enumerate(zip(top_k_perturbs, top_k_rewards))
        ],
    }
    ...
    results = {
        ...
        "top_k_perturbs": [(int(s), float(sig), bool(neg)) for s, sig, neg in top_k_perturbs],
...
def main(args):
    ...
    # Launch engines
    engines, pgs = launch_engines(args.num_engines, base_model_path, precision=args.precision, tensor_parallel_size=args.tp, gpu_memory_utilization=0.35)
    ...
    try:
        if not is_resume:
            ...
            top_k_perturbs = [key for key, _ in sorted_perturbs[:args.max_top_k]]
            top_k_rewards = [reward for _, reward in sorted_perturbs[:args.max_top_k]]
            
            print(f"Selected top-{args.max_top_k} from {len(perf)} perturbations")
            print(f"\n=== Top-10 Perturbations ===")
            for i, ((seed, sigma, neg), reward) in enumerate(sorted_perturbs[:10]):
                print(f"  {i+1}. seed={seed}, σ={sigma}, neg={neg}: {reward:.4f}")
        else:
            print(f"\n=== Loaded Top-{len(top_k_perturbs)} Perturbations ===")
            for i, ((seed, sigma, neg), reward) in enumerate(zip(top_k_perturbs[:10], top_k_rewards[:10])):
                print(f"  {i+1}. seed={seed}, σ={sigma}, neg={neg}: {reward:.4f}")
        ...
        if args.robust_data:
            print(f"\n{'='*60}\nROBUSTNESS EVALUATION\n{'='*60}")
            with open(args.robust_data, 'r') as f:
                robust_raw = json.load(f)
            
            if args.test_samples:
                robust_raw = robust_raw[:args.test_samples]
            
            for variant in ['adv', 'ood']:
                print(f"\n>>> Evaluating {variant.upper()} robustness...")
                v_prompts = [f"Question: {d[f'question_{variant}']}\nAnswer: " for d in robust_raw]
                v_datas = [{"ground_truth": d['ground_truth']} for d in robust_raw]
                
                # Base model
                v_base_out = ray.get(engines[0].generate.remote(v_prompts, sampling_params, use_tqdm=False))
                v_base_acc = handler.postprocess_outputs(v_base_out, v_datas)
                print(f"Base {variant} Accuracy: {v_base_acc*100:.2f}%")
                
                # Ensemble
                v_ensemble = run_ensemble_evaluation(args, engines, handler, v_prompts, v_datas, top_k_perturbs, sampling_params, v_base_acc)
                ensemble_results[f"robust_{variant}"] = v_ensemble
